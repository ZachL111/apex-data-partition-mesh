...
`apex-data-partition-mesh` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies partition behavior through seeded input scenarios, with deterministic summary checks and no network dependency.
